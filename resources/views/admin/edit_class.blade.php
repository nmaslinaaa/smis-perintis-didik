...
<div class="main-content">
    ...
    <div class="class-list-container" style="max-width:700px;">
        <div class="class-list-title" style="font-size:2rem;">Edit Class <span style="font-size:1.3rem; font-weight:400;">({{ $class->class_name ?? '' }})</span></div>
        <div class="class-list-divider" style="margin-bottom:30px;"></div>
        <form method="POST" action="{{ url('/admin/edit_class/' . ($class->classID ?? $class['classID']) . '/update') }}">
            @csrf
            <div style="margin-bottom: 28px;">
                ...
                <div id="existing_classes" style="color: #666; font-size: 0.9rem; margin-top: 5px;">
                    ...
                    @php
                        $existingClasses = \App\Models\Classes::where('classID', '!=', $class->classID ?? $class['classID'])->pluck('class_name')->toArray();
            ...
            <div style="margin-bottom: 32px;">
                ...
                <div style="display: flex; align-items: center; margin-bottom: 10px;">
                    <select id="teacher_select" style="width: 350px; padding: 10px; font-size: 1.1rem; border: 1px solid #ccc; border-radius: 5px;">
                        @foreach($teachers as $teacher)
                            @php
                                $alreadyAssigned = $assignedTeachers->contains('employeeID', $teacher->employeeID);
                            @endphp
                            @if(!$alreadyAssigned)
                ...
                <div id="assigned_teachers_list">
                    @foreach($assignedTeachers as $assigned)
                    <div class="assigned-teacher-row" style="display: flex; align-items: center; margin-bottom: 10px;">
                        <select disabled style="width: 350px; padding: 10px; font-size: 1.1rem; border: 1px solid #ccc; border-radius: 5px; background: #f7f7f7; color: #333;">
                            <option>{{ $assigned->firstname }} {{ $assigned->lastname }}</option>
                        </select>
                        <button type="button" class="remove-teacher-btn" style="margin-left: 10px; background: none; border: none; cursor: pointer;">
                            <span style="font-size: 2rem; color: #f44336; display: flex; align-items: center;"><i class="fas fa-times-circle"></i></span>
                        </button>
                        <input type="hidden" name="assigned_teachers[]" value="{{ $assigned->employeeID }}">
                    </div>
                    @endforeach
                </div>
            </div>
            <div style="margin-top: 40px; display: flex; gap: 30px;">
                <button type="submit" style="background: #19d219; color: #fff; font-weight: bold; font-size: 1.1rem; border: none; border-radius: 5px; padding: 12px 40px; cursor: pointer;">Update</button>
                <button type="button" style="background: #f44336; color: #fff; font-weight: bold; font-size: 1.1rem; border: none; border-radius: 5px; padding: 12px 40px; cursor: pointer;" onclick="window.location='{{ url('/admin/manage_classes') }}'">Cancel</button>
            </div>
        </form>
    </div>
</div>
<script>
    ...
    // Existing classes for validation (excluding current class)
    ...
    document.getElementById('class_name').addEventListener('input', function() {
        ...
        // Check for duplicates (case-insensitive), but allow the current class name
    ...
    // Function to update teacher select options
    function updateTeacherSelect() {
        const select = document.getElementById('teacher_select');
        const assignedTeachers = Array.from(document.querySelectorAll('input[name="assigned_teachers[]"]'))
            .map(input => input.value);

        // Clear current options
        select.innerHTML = '';

        // Add only unassigned teachers
        ...
        // Disable add button if no teachers available
        const addButton = document.getElementById('add_teacher_btn');
        if (select.options.length === 0) {
            select.innerHTML = '<option value="">No more teachers available</option>';
    ...
    document.getElementById('add_teacher_btn').onclick = function() {
        ...
        if (!id) return; // Skip if no value selected
        ...
        row.innerHTML = `
            ...
            <button type="button" class="remove-teacher-btn" style="margin-left: 10px; background: none; border: none; cursor: pointer;">
                <span style="font-size: 2rem; color: #f44336; display: flex; align-items: center;">
        ...
        row.querySelector('.remove-teacher-btn').onclick = function() {
            row.remove();
            updateTeacherSelect(); // Update dropdown after removing
        };
    ...
    // Remove teacher (untuk row sedia ada)
    document.querySelectorAll('.remove-teacher-btn').forEach(function(btn){
        btn.onclick = function(){
            btn.closest('.assigned-teacher-row').remove();
            updateTeacherSelect(); // Update dropdown after removing
        };
    });

    // Initial update of teacher select
    updateTeacherSelect();
</script>
