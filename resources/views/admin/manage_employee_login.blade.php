...
<head>
    ...
    <title>@yield('title', 'Manage Employee Login')</title>
    ...
    <style>
       ...
       .stats {
        ...
        .btn-edit-employee.saved,
        .btn-edit-employee.saved:hover {
            background: #43b324 !important;
            color: #fff !important;
            cursor: pointer;
        }
    </style>
...
<body>
   ...
   <div class="main-content">
        <div style="background: #fff; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); padding: 36px 36px 24px 36px; max-width: 1200px; margin: 0 auto;">
            <h2 style="font-size: 32px; font-weight: 700; margin: 0 0 8px 0;">Manage Employee Login</h2>
            ...
            <table style="width: 100%; border-collapse: collapse; background: #fff;">
                ...
                <tbody>
                    ...
                    @forelse($employees as $index => $employee)
                        <tr>
                            <td style="border: 1px solid #e0e0e0; padding: 12px 8px; font-size: 16px;">{{ $index + 1 }}</td>
                            <td style="border: 1px solid #e0e0e0; padding: 12px 8px; font-size: 16px;">{{ $employee->firstname }}</td>
                            <td style="border: 1px solid #e0e0e0; padding: 12px 8px; font-size: 16px;">{{ $roles[$employee->group_level] ?? 'Unknown' }}</td>
                            <td style="border: 1px solid #e0e0e0; padding: 12px 8px; font-size: 16px;">
                                <input name="user_name" type="text" value="{{ $employee->user_name }}" style="width: 100px; background: #eaeaea; border: none; border-radius: 6px; padding: 6px 10px; font-size: 15px; text-align: center;" />
                            </td>
                            <td style="border: 1px solid #e0e0e0; padding: 12px 8px; font-size: 16px;">
                                <div style="display: flex; align-items: center;">
                                    <input name="password" type="password" value="{{ $employee->password }}" class="employee-password" style="width: 80px; background: #eaeaea; border: none; border-radius: 6px; padding: 6px 10px; font-size: 15px; text-align: center;" />
                            ...
                            <td style="border: 1px solid #e0e0e0; padding: 12px 8px; font-size: 16px;">
                                <button type="button" class="action-btn btn-edit-employee" style="background:#2196f3; color:#fff; padding: 6px 18px; border-radius: 6px; font-size: 15px; font-weight: 600; margin-right: 8px;" data-employee-id="{{ $employee->employeeID }}">Save</button>
                                <button type="button" class="btn-whatsapp" style="background: #25d366; color: #fff; border: none; border-radius: 6px; padding: 8px 12px; font-size: 16px; cursor: pointer; margin-left: 4px;" data-employee-id="{{ $employee->employeeID }}" data-employee-name="{{ $employee->firstname }} {{ $employee->lastname }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" style="text-align:center; padding: 30px;">No employee data.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        // Toggle password visibility for employee
        document.querySelectorAll('.toggle-password').forEach(function(btn){
            btn.onclick = function(){
                const input = btn.parentNode.parentNode.querySelector('.employee-password');
        ...
        // Save only the current employee row (AJAX, green on success, blue on edit, no error color, only if changed)
        document.querySelectorAll('.btn-edit-employee').forEach(function(btn){
            const row = btn.closest('tr');
            const usernameInput = row.querySelector('input[name="user_name"]');
            const passwordInput = row.querySelector('.employee-password');
            ...
            btn.onclick = function(){
                ...
                fetch(`/admin/update_employee_login/${employeeId}`, {
                    method: 'POST',
                    headers: {
        ...
        // WhatsApp button functionality for employee login credentials
        document.querySelectorAll('.btn-whatsapp').forEach(function(btn) {
            btn.onclick = function() {
                ...
                fetch(`/admin/send_employee_login_credentials/${employeeId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'WhatsApp Sent!',
                            text: `Login credentials sent successfully to ${employeeName}`,
                            timer: 2000,
                            showConfirmButton: false
                        });
                        setTimeout(() => {
