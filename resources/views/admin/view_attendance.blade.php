...
<head>
    ...
    <style>
       ...
       body {
        ...
        .filters select[disabled], .filters input[type=date][disabled] { background: #f7f7f7; color: #aaa; }
    ...
    <div class="main-content">
        <div class="card">
            <div class="card-header">
                <div class="card-title">View Student Attendance</div>
            </div>
                        ...
                        @if($selectedClassID && $selectedSubjectID && $selectedSlotID)
                            @forelse($students as $i => $student)
                                <tr>
                                    ...
                                    @php
                                        $status = $attendanceData[$student->studentID] ?? null;
                                        $class = $status === 'Present' ? 'att-present' : ($status === 'Absent' ? 'att-absent' : ($status === 'Leave with permission' ? 'att-leave' : 'att-empty'));
                                    @endphp
                                    <td><span class="att-status {{ $class }}">{{ $status ? ($status === 'Leave with permission' ? 'Leave' : $status) : '-' }}</span></td>
                                </tr>
                            @empty
                                <tr><td colspan="3" style="text-align:center;">No students found for this slot.</td></tr>
                            @endforelse
                        @endif
                    </tbody>
                </table>
