...
<div class="main-content" style="margin-top: 60px; margin-left: 220px; padding: 0; min-height: 100vh; background: #f4f7fa;">
    <div style="max-width: 900px; margin: 40px auto 0 auto; background: #fff; border-radius: 14px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); padding: 40px 40px 32px 40px;">
        <div style="font-size: 2.2rem; font-weight: 700; margin-bottom: 18px; border-bottom: 3px solid #edc10c; padding-bottom: 8px;">
            {{ $profile->firstname ?? $profile->name ?? '' }}’s Profile
        </div>
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 32px;">
            ...
            <div style="text-align: center; flex: 1;">
                ...
                <div style="font-size: 1.5rem; font-weight: 700; margin-top: 10px;">
                    {{ $profile->firstname ?? $profile->name ?? '' }}{{ isset($profile->lastname) ? ' ' . $profile->lastname : '' }}
                </div>
                @if(isset($profile->ic_number))
                <div style="font-size: 1.1rem; color: #222; font-weight: 500; margin-top: 2px;">
                    IC Number : {{ $profile->ic_number }}
                </div>
                @endif
            </div>
            <div></div>
        </div>
        <div style="background: #eaf6fb; border: 2px solid #90caf9; border-radius: 14px; padding: 32px 40px; margin: 0 auto 32px auto; max-width: 600px; font-size: 1.18rem; font-weight: 600; color: #222;">
            <div style="margin-bottom: 12px;">Email : {{ $profile->email ?? '' }}</div>
            <div style="margin-bottom: 12px;">Phone Number : {{ $profile->phonenumber ?? '' }}</div>
            @if($role !== 'Parent')
                <div style="margin-bottom: 12px;">Address : {{ $profile->address ?? '' }}</div>
                @if(isset($profile->educational_background))
                    <div style="margin-bottom: 12px;">Educational Background : {{ $profile->educational_background }}</div>
                @endif
                @if(isset($profile->current_job))
                    <div style="margin-bottom: 12px;">Current Job : {{ $profile->current_job }}</div>
                @endif
            @endif
            <div>Role : {{ $role }}</div>
        </div>
        ...
        <div style="margin-top: 40px;">
            ...
            @if($child->verification_status === 'approved')
            <div style="background: #fffbe9; border: 2px solid #ffe066; border-radius: 14px; padding: 24px 32px; margin-bottom: 24px; max-width: 700px; font-size: 1.08rem; font-weight: 600; color: #222;">
                ...
                <div style="margin-bottom: 6px;"><span style="font-weight: 500;">School Name:</span> {{ $child->school_name ?? '-' }}</div>
                <div style="margin-bottom: 6px;"><span style="font-weight: 500;">Address:</span> {{ $child->address ?? '-' }}</div>
                <div style="margin-bottom: 6px;"><span style="font-weight: 500;">Class:</span> {{ $child->class ? $child->class->class_name : '-' }}</div>
                <div style="margin-bottom: 6px;"><span style="font-weight: 500;">Subject Taken:</span> @if($child->subjects && $child->subjects->count()){{ $child->subjects->pluck('subject_name')->join(', ') }}@else-@endif</div>
                <div style="margin-bottom: 6px;"><span style="font-weight: 500;">Fee Amount (RM):</span>
                    @php
                        $totalFee = 0;
                        if($child->studentSubjects && $child->studentSubjects->count()) {
                            foreach($child->studentSubjects as $studentSubject) {
                                if($studentSubject->classsubjectID) {
                                    ...
                                    if($classSubject) {
                                        $totalFee += $classSubject->subject_price;
                                    }
                                }
                            }
                        }
                ...
                <div style="margin-bottom: 6px;"><span style="font-weight: 500;">Start Date:</span> {{ $child->tuition_startdate ?? '-' }}</div>
                <div><span style="font-weight: 500;">Status:</span> <span style="font-weight:700; padding:4px 18px; border-radius:8px; display:inline-block; @if($child->student_status == 1) background:#e6ffe6; color:#19d219; border:1.5px solid #19d219; @else background:#ffeaea; color:#f44336; border:1.5px solid #f44336; @endif">{{ $child->student_status == 1 ? 'Active' : 'Inactive' }}</span></div>
            </div>
            @endif
            @empty
            <div style="background: #fffbe9; border: 2px solid #ffe066; border-radius: 14px; padding: 24px 32px; margin-bottom: 24px; max-width: 700px; font-size: 1.08rem; font-weight: 600; color: #222; text-align:center;">No children found.</div>
            @endforelse
        </div>
        @endif
        <div style="text-align: left; margin-top: 18px;">
            <a href="{{ url('/edit_manage_profile') }}" style="background: #43b324; color: #fff; font-size: 1.15rem; font-weight: 700; border: none; border-radius: 6px; padding: 12px 48px; cursor: pointer; text-decoration: none;">Update</a>
        </div>
    </div>
</div>
<script>
